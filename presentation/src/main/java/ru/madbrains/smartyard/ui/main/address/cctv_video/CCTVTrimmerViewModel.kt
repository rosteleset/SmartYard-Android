...
//                newTo = newFrom.plusSeconds(TimeInterval.TIME_OFFSET * 2)
                newTo = newFrom.plusSeconds(TimeInterval.TIME_OFFSET * 6) //TODO При выборе фрагиента был временной периуд в 1 час, сделал в 4 часа
            } else if (newTo > availableRanges.last().endDate) {
                newTo = availableRanges.last().endDate
//                newFrom = newTo.plusSeconds(-TimeInterval.TIME_OFFSET * 2) //TODO
                newFrom = newTo.plusSeconds(-TimeInterval.TIME_OFFSET * 2)
            }else{
                newFrom = centerTime
                newTo = if (centerTime.plusSeconds(TimeInterval.TIME_OFFSET * 6) > availableRanges.last().endDate)  availableRanges.last().endDate
                else centerTime.plusSeconds(TimeInterval.TIME_OFFSET * 6)
            } //TODO
        }
