...
class AddressFragment : Fragment(), GuestAccessDialogFragment.OnGuestAccessListener {
    ...
    private val delegate: AddressDelegate? by injectOrNull(AddressDelegate::class.java)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val onScrollListener = if (mViewModel.entranceView == EntrancesView.LIST) {
            null
        } else {
            object : RecyclerView.OnScrollListener() {
                override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                    ...
                    if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                        createWebRtcConnectionForTopmostExpandedItem()
                    }
                }
            }
        }
        initAddressList(onScrollListener)
        bindViews()
        initActivityObservers()
        initObservers()
        delegate?.extendConfig(findNavController(), binding, mViewModel,
            viewLifecycleOwner, onScrollListener)
    }

    private fun initAddressList(onScrollListener: RecyclerView.OnScrollListener?) {
        ...
        binding.addressList.let { rw ->
            rw.layoutManager = layoutManager

            rw.adapter = adapter

            rw.applyBottomNavInsetsToPadding()
            ...
            onScrollListener?.let { rw.addOnScrollListener(it) }
        }
    ...
    private fun createWebRtcConnectionForTopmostExpandedItem() {
    ...
    private fun onIssueAction(action: IssueAction) {
        if (delegate?.onIssueAction(findNavController(), action) == true) {
            return
        }
    ...
    private fun bindViews() {
        ...
        binding.tvEmptyList.applyBottomNavInsetsToMargin()
    }
