...
class AvailableServicesViewModel(
    ...
    fun availableOkBtn(servicesList: List<AvailableModel>) {
        _stateEnabledButtonNext.value = Event(!servicesList.none { it.isChecked })
    }

    fun checkServices(servicesList: List<AvailableModel>, address: String) {
        // Количество общедомовых сервисов
        val commonHouse = servicesList.filterCommon().size

        // Количество выбранных других сервисов
        val otherSelect = servicesList.filterOther().size
        ...
        if (commonHouse > 0 && otherSelect == 0) {
            ...
            issueGoToTheOfficeMy(
                address,
                servicesList.filterOther().joinTitles()
            )
    ...
    companion object {
        private val origin = IssueOrigin.AVAILABLE_SERVICES
        private val originOffice = IssueOrigin.AVAILABLE_SERVICES_OFFICE

        fun List<AvailableModel>.filterCommon() = filter { it.isMandatory }
        fun List<AvailableModel>.filterOther() = filter { !it.isMandatory && it.isChecked }
        fun List<AvailableModel>.joinTitles() = joinToString { "\'${it.title}\'" }
    }
}
