...
open class CCTVArchivePlayerFragment : Fragment(), UserInteractionListener, ExitFullscreenListener {
    private var _binding: FragmentCctvArchivePlayerBinding? = null
    protected val binding get() = _binding!!
    private var adapter: TimeFragmentButtonsAdapter? = null
    ...
    private fun setFullscreenMode() {
        ...
        moveRangeSliderToMainActivity()

        removeTrimAndMainAction()
    ...
    protected open fun moveRangeSliderToMainActivity() {
    ...
    protected open fun removeTrimAndMainAction() {
    ...
    private fun setNormalMode() {
        if (activity?.requestedOrientation != ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
            ...
            moveRangeSliderBackToFragment()

            addTrimAndMainAction()
            ...
            binding.svContentWrap.background =
                ColorDrawable(ContextCompat.getColor(requireContext(), R.color.dialog_background))
            binding.flVideoPlayback.setBackgroundColor(ContextCompat.getColor(requireContext(), R.color.dialog_background))
            binding.llControls.background =
                ColorDrawable(ContextCompat.getColor(requireContext(), R.color.dialog_background))
    ...
    protected open fun moveRangeSliderBackToFragment() {
        (binding.rangePlayer.parent as ViewGroup).removeView(binding.rangePlayer)
        binding.videoWrap.addView(binding.rangePlayer, 2)
    }

    protected open fun setRangeSliderLayoutParams() {
        binding.rangePlayer.layoutParams = lpRangeSlider
        binding.rangePlayer.requestLayout()
    }

    protected open fun addTrimAndMainAction() {
    ...
    private fun setupUi(context: Context) {
        ...
        binding.rvTimeFragmentButtons.let {
            ...
            toggleMainActionVisibility(adapter?.itemCount ?: 0)
        }
    ...
    protected open fun toggleMainActionVisibility(itemCount: Int) {
        binding.btnMainAction.isVisible = itemCount > 0
    }
    ...
    private fun setupObserve(context: Context) {
        ...
        mViewModel.alertEvent.observe(
            viewLifecycleOwner,
            EventObserver { type ->
                if (type == CCTVArchivePlayerViewModel.dialogPrepareVideo) {
                    showStandardAlert(
                        context,
