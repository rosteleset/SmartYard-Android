...
data class Plog(
    ...
    val event: String
        get() {
            return when (eventType) {
                ...
                EVENT_OPEN_BY_KEY -> "Открытие ключом"
                EVENT_OPEN_FROM_APP -> "Открытие из приложения"
                EVENT_OPEN_BY_FACE -> "Открытие по лицу"
                EVENT_OPEN_BY_CODE -> "Открытие по коду"
                EVENT_OPEN_GATES_BY_CALL -> "Открытие звонком"
                EVENT_OPEN_BY_LINK -> "Открытие по ссылке"
    ...
    companion object {
        ...
        const val EVENT_OPEN_BY_LINK = 9
